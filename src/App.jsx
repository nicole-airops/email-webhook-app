...
import { Accordion, AccordionSection, Button, TextInput, Select } from '@frontapp/ui-kit';
import './App.css';
...
// Transform for UI Kit Select component which expects { title, value, icon? }
const formatOptionsForSelect = baseFormatOptions.map(opt => ({
  value: opt.value,
  title: opt.label,
  // icon: could be mapped to an <Icon name="..."/> component if desired
}));
...
function App() {
  ...
  const [selectedFormatValue, setSelectedFormatValue] = useState(''); // Stores the value from Select
  ...
  // const [textareaHeight, setTextareaHeight] = useState(60); // Managed by TextInput or CSS
  // const [isTextareaResizing, setIsTextareaResizing] = useState(false); // Removed with custom resize
  const [containerSize, setContainerSize] = useState({ width: 0, height: 0 });

  const cardRef = useRef(null);
  // const textareaRef = useRef(null); // Not needed if TextInput manages its own ref
  // const textareaContainerRef = useRef(null); // Removed with custom resize
  ...
  const loadTaskResultsFromStorage = useCallback((conversationId) => {
    ...
    // Initialize polling for pending tasks from storage
    ...
    if (pendingStoredTasks.size > 0) {
        addDebugLog(`⏳ Resuming polling for ${pendingStoredTasks.size} tasks from storage.`);
    }
  ...
  useEffect(() => {
    ...
    if (conversationId) {
      ...
      loadTaskResultsFromStorage(conversationId); // This will also set up pollingTasks
    } else {
  ...
  const checkTaskStatus = useCallback(async (taskId) => {
    ...
    if (!currentConversationId) {
        ...
        setPollingTasks(prev => { const newSet = new Set(prev); newSet.delete(taskId); return newSet; });
        return;
    }
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          setPollingTasks(prev => { const newSet = new Set(prev); newSet.delete(taskId); addDebugLog(`🚫 Task ${taskId} (404), removing from poll.`); return newSet; });
          setTaskResults(prevTaskResults => {
            const updatedTasks = prevTaskResults.map(task => task.id === taskId && (task.status === 'pending' || task.status === 'processing') ? { ...task, status: 'failed', result: 'Task not found by status API (404).', completedAt: new Date().toISOString() } : task);
      ...
      if (result.status === 'completed' || result.status === 'failed' || result.status === 'error') {
        ...
        setTaskResults(prevTaskResults => {
          const updatedTasks = prevTaskResults.map(task => {
            if (task.id === taskId) {
              taskForLinkDetails = { ...task, status: result.status === 'completed' ? 'completed' : 'failed', result: result.status === 'completed' ? result.data : (result.error || result.message || 'Task processing failed'), completedAt: result.completedAt || new Date().toISOString() };
        ...
        if (result.status === 'completed') {
          ...
          if (taskForLinkDetails && context?.addLink && context?.conversation) {
            try {
              ...
              const formatLabel = taskForLinkDetails.selectedFormatValue ? baseFormatOptions.find(f => f.value === taskForLinkDetails.selectedFormatValue)?.label.replace(/[📧•📊📄✅📋📅✏️{}]/g, '').trim() : taskForLinkDetails.outputFormat;
            ...
            } catch (linkError) { addDebugLog(`❌ Error adding completion link for ${taskId}: ${linkError.message}`); }
          }
        } else { setStatus(`Task ${simpleTaskId} ${result.status}.`); }
        setPollingTasks(prev => { const newSet = new Set(prev); newSet.delete(taskId); addDebugLog(`🛑 Stopped polling for task ${taskId} (status: ${result.status})`); return newSet; });
      }
    } catch (error) { addDebugLog(`💥 Error in checkTaskStatus for ${taskId}: ${error.message}`); console.error(`Error checking task status for ${taskId}:`, error); }
  }, [context, addDebugLog, saveTaskResultsToStorage]); // Dependencies refined

  useEffect(() => {
    if (pollingTasks.size === 0) return undefined;
    ...
    pollingTasks.forEach(taskId => {
      ...
      const intervalId = setInterval(() => checkTaskStatus(taskId), 30000); 
      intervalIds.set(taskId, intervalId);
    });
    return () => { addDebugLog(`🧹 Clearing ${intervalIds.size} polling intervals.`); intervalIds.forEach(intervalId => clearInterval(intervalId)); };
  ...
  useEffect(() => {
    const observeContainerSize = () => {
      ...
      const updateSize = () => {
        ...
        setCardSize(prev => ({ width: Math.min(prev.width, maxWidth), height: Math.min(prev.height, maxHeight) }));
      };
      updateSize();
      if (window.ResizeObserver) {
        const resizeObserver = new ResizeObserver(updateSize); resizeObserver.observe(parent);
  ...
  const containerStyleName = isComposer ? 'composer' : 'sidebar';

  const textSizes = useMemo(() => {
    const baseSize = Math.max(11, Math.min(14, cardSize.width / 28));
    return { base: `${baseSize}px`, small: `${baseSize - 1}px`, tiny: `${baseSize - 2}px`, header: `${baseSize + 1}px` };
  }, [cardSize.width]);
  ...
  // const handleTextareaResizeStart = useCallback((e) => { e.preventDefault(); e.stopPropagation(); setIsTextareaResizing(true); }, []); // Removed

  useEffect(() => {
    // This effect updates `outputFormat` when `selectedFormatValue` (from <Select>) changes,
    // unless 'custom' is selected.
    if (selectedFormatValue && selectedFormatValue !== 'custom') {
      const selectedOption = baseFormatOptions.find(opt => opt.value === selectedFormatValue);
      if (selectedOption) {
        setOutputFormat(selectedOption.label.replace(/[📧•📊📄✅📋📅✏️{}]/g, '').trim());
      }
    } else if (selectedFormatValue === '') { // Clear outputFormat if "Select format..." is chosen
        setOutputFormat('');
    }
    // If selectedFormatValue is 'custom', outputFormat is managed by its own TextInput
  }, [selectedFormatValue]);


  const getCurrentOutputFormat = useCallback(() => {
    if (selectedFormatValue === 'custom') {
      return outputFormat.trim();
    }
    const selectedOption = baseFormatOptions.find(opt => opt.value === selectedFormatValue);
    return selectedOption ? selectedOption.label.replace(/[📧•📊📄✅📋📅✏️{}]/g, '').trim() : '';
  }, [selectedFormatValue, outputFormat]);


  const createCombinedInstructions = useCallback(() => {
    let combinedText = comment.trim();
    if (mode === 'task') {
      const currentActualOutputFormat = getCurrentOutputFormat();
      if (currentActualOutputFormat) {
        combinedText += `\n\nOutput Format: ${currentActualOutputFormat}`;
      }
      if (uploadedFile) {
    ...
    return combinedText;
  }, [comment, mode, uploadedFile, getCurrentOutputFormat]);

  const createSinglePayload = useCallback((instructions, conversationData, taskIdForPayload) => {
    const payload = {
      request: {
        instructions: instructions, mode: mode,
        context: { type: context?.type, conversation: conversationData, teammate: context?.teammate },
        ...(taskIdForPayload && { taskId: taskIdForPayload }),
        ...(uploadedFile && { attached_file: { name: uploadedFile.name, type: uploadedFile.type, size: uploadedFile.size, content_preview: uploadedFile.preview }})
      }
    };
  ...
  const handleFileUpload = useCallback(async (event) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      setStatus('File too large (max 2MB)'); addDebugLog(`📎 File ${file.name} rejected: too large`); return;
    }
    ...
    if (file.type.startsWith('text/') || file.name.endsWith('.json') || file.name.endsWith('.csv') || file.name.endsWith('.md')) {
      try {
        ...
        setUploadedFile(fileData); setStatus(`📎 ${file.name} uploaded`); addDebugLog(`📎 File ${file.name} uploaded with preview.`);
      } catch (readError) { setStatus('File read failed'); addDebugLog(`🔴 File ${file.name} read error: ${readError.message}`); setUploadedFile({ ...fileData, preview: 'Error reading file content.' }); }
    } else { setUploadedFile(fileData); setStatus(`📎 ${file.name} uploaded (binary)`); addDebugLog(`📎 File ${file.name} (binary) uploaded.`); }
  }, [addDebugLog]); // setStatus, setUploadedFile are stable
  ...
  }, [addDebugLog, uploadedFile?.name]); // setStatus, setUploadedFile are stable

  const processRequest = useCallback(async () => {
    const now = Date.now();
    if (isProcessingRef.current || (now - lastCallTimeRef.current) < 1000) { addDebugLog('🚫 Prevented duplicate call'); return; }
    if (!comment.trim()) { setStatus('Add instructions'); return; }
    
    const currentActualOutputFormat = getCurrentOutputFormat();
    if (mode === 'task' && !currentActualOutputFormat) { setStatus('Select or enter output format'); return; }
    
    isProcessingRef.current = true; lastCallTimeRef.current = now;
    setIsSending(true); setStatus('Processing...'); addDebugLog(`🚀 Starting ${mode} request...`);
    ...
    try {
      ...
      let conversationData = {}; const conversationId = context?.conversation?.id;

      if (conversationId && context.conversation) {
        conversationData = { id: conversationId, subject: context.conversation.subject, status: context.conversation.status, recipient: context.conversation.recipient?.handle, tags: context.conversation.tags?.map(t => t.name), assignee: context.conversation.assignee?.username };
        try {
          const messagesResponse = await context.listMessages({ limit: 10 });
          conversationData.messages = messagesResponse.results.map(msg => ({ id: msg.id, type: msg.type, direction: msg.direction, author: msg.author?.username, recipient: msg.recipient?.handle, subject: msg.subject, bodyPreview: msg.text?.substring(0, 200) + (msg.text?.length > 200 ? '...' : ''), createdAt: msg.createdAt, }));
          addDebugLog(`📧 Loaded ${conversationData.messages.length} messages for context.`);
        } catch (err) { addDebugLog(`❌ Error loading messages: ${err.message}`); }
        
        const newHistoryEntry = { text: comment, mode: mode, outputFormat: currentActualOutputFormat, selectedFormatValue: selectedFormatValue, hasFile: !!uploadedFile, fileName: uploadedFile?.name, timestamp: new Date().toISOString(), user: context.teammate?.username || 'Unknown user' };
        ...
        if (mode === 'task' && clientSideTaskId) {
          const newTask = { id: clientSideTaskId, comment: comment, outputFormat: currentActualOutputFormat, selectedFormatValue: selectedFormatValue, hasFile: !!uploadedFile, fileName: uploadedFile?.name, status: 'pending', createdAt: new Date().toISOString(), user: context.teammate?.username || 'Unknown user' };
          ...
          try {
            if (context.addLink) {
              ...
              const linkName = `⏳ AirOps Task: ${currentActualOutputFormat || 'Processing...'} ${uploadedFile ? '📎' : ''}`;
              await context.addLink(linkUrl, linkName); addDebugLog(`🔗 Pending task link added to Front for ${clientSideTaskId}`);
            }
            ...
            try {
                if (context.addLink) {
                    ...
                    await context.addLink(linkUrl, linkName); addDebugLog('🔗 Email generation link added to Front conversation');
                }
            } catch (linkError) { addDebugLog(`❌ Error adding email link: ${linkError.message}`); }
        }
      }
      if (context?.draft) { conversationData.draft = { body: context.draft.body, subject: context.draft.subject }; }
      ...
      for (let i = 0; i < payloadOptions.length; i++) {
        ...
        try {
          ...
          responseDataText = await response.text(); addDebugLog(`📈 Approach ${i + 1} response status: ${response.status}`);
          if (response.ok) { addDebugLog(`✅ Payload approach ${i + 1} worked! Response (first 100 chars): ${responseDataText.substring(0,100)}`); break; }
          else { addDebugLog(`❌ Approach ${i + 1} failed: ${response.status} - ${responseDataText.substring(0, 100)}`); if (i === payloadOptions.length - 1) throw new Error(`All payload approaches failed. Last error (${response.status}): ${responseDataText}`); }
        } catch (error) { addDebugLog(`💥 Approach ${i + 1} fetch/network error: ${error.message}`); if (i === payloadOptions.length - 1) throw error; }
      }
      if (response && response.ok) {
        if (mode === 'email') { setStatus('Email instructions sent!'); }
        else { setStatus('Task created and polling started.'); if (clientSideTaskId) { setPollingTasks(prev => new Set(prev).add(clientSideTaskId)); addDebugLog(`⏳ Task ${clientSideTaskId} added to polling queue.`); }}
        setComment('');
        if (selectedFormatValue === 'custom') setOutputFormat(''); 
        setSelectedFormatValue(''); 
        removeFile();
      } else { setStatus('Request failed. See debug log.'); addDebugLog('🔴 Webhook request ultimately failed.'); }
    } catch (error) { addDebugLog(`💥 Error during request processing: ${error.message}`); setStatus('Error: ' + error.message.substring(0, 100) + (error.message.length > 100 ? '...' : '')); }
    finally { setIsSending(false); isProcessingRef.current = false; addDebugLog('🏁 Request processing finished.'); }
  }, [
    comment, mode, outputFormat, selectedFormatValue, uploadedFile, context, addDebugLog,
    createCombinedInstructions, createSinglePayload, saveCommentHistoryToStorage,
    saveTaskResultsToStorage, removeFile, getCurrentOutputFormat // Removed state setters from deps, they are stable
  ]);

  const formatDate = useCallback((isoString) => {
    if (!isoString) return 'Date N/A';
    try { const date = new Date(isoString); if (isNaN(date.getTime())) return 'Invalid Date'; return date.toLocaleDateString(undefined, { month: 'short', day: 'numeric' }); }
    catch (e) { return String(isoString).substring(0,10); }
  }, []);

  const insertIntoDraft = useCallback((content) => {
    if (!context) { setStatus('Cannot insert: Front context not available.'); addDebugLog('⚠️ Cannot insert: Front context not available.'); return; }
    ...
    if (context.insertTextIntoBody && (context.type === 'messageComposer' || context.type === 'messageViewer')) {
      context.insertTextIntoBody(plainTextContent); setStatus('Inserted into draft!'); addDebugLog('✅ Inserted content into draft.');
    } else if (context.createDraft && context.type === 'messageViewer') {
      context.createDraft({ content: { body: plainTextContent, type: 'text' } }); setStatus('New draft created with content!'); addDebugLog('✅ New draft created with content.');
    } else {
  ...
  }, [context, addDebugLog]); // setStatus is stable

  if (!context) {
    return ( <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', padding: '20px', textAlign: 'center', color: '#9ca3af' }}> Loading AirOps Plugin context... </div> );
  }
  if (context.type === 'messageComposer' && !context.conversation) {
    return ( <div style={{ padding: '16px', fontSize: textSizes.base, color: '#64748b', textAlign: 'center' }}> This plugin requires an active conversation context. </div> );
  }
  
  const currentActualOutputFormat = getCurrentOutputFormat();
  const isSendButtonDisabled = isSending || !comment.trim() || (mode === 'task' && !currentActualOutputFormat);
  ...
  return (
    <div 
      ...
      style={{
        ...
        fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', // Consider using Front's font variables if available
        position: 'relative', overflow: 'hidden',
        boxShadow: '0 2px 8px rgba(0, 0, 0, 0.1)',
        display: 'flex', flexDirection: 'column',
        transition: isResizing ? 'none' : 'width 0.2s ease, height 0.2s ease'
      }}
      className={`airops-app-container ${containerStyleName}`}
    >
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: '12px', fontSize: textSizes.header, fontWeight: '600', color: '#111827', flexShrink: 0 }}>
        ...
        <span style={{ marginLeft: 'auto', fontSize: textSizes.tiny, color: '#94a3b8', fontWeight: 'normal', whiteSpace: 'nowrap' }}>
          {containerStyleName} {Math.round(cardSize.width)}×{Math.round(cardSize.height)}
        </span>
      </div>

      <div style={{ display: 'flex', marginBottom: '12px', borderRadius: '6px', flexShrink: 0, gap: '2px' }}>
         <Button 
            label="Email"
            type={mode === 'email' ? 'primary' : 'default'}
            onClick={() => setMode('email')}
            style={{ flex: 1 }} // Adjust styling as Button component might not take all button styles
         />
         <Button
            label="Task"
            type={mode === 'task' ? 'primary' : 'default'}
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingRight: '4px', marginBottom: '12px', minHeight: 0 }}>
        ...
        <TextInput
            ...
            // Removed custom resize, TextInput might auto-grow or have height props
            // style={{ height: `${textareaHeight}px`, paddingBottom: '20px' }}
            style={{ marginBottom: '8px', minHeight: '60px' }} // Basic styling
        />
        ...
        {mode === 'task' && (
          <div style={{ marginBottom: '12px' }}>
            <Select
              ...
              onOptionPress={(option) => setSelectedFormatValue(option.value)}
              options={formatOptionsForSelect}
              placeholder="Select output format..."
              style={{ marginBottom: '8px', width: '100%'}}
            />

            {selectedFormatValue === 'custom' && (
              <TextInput
                value={outputFormat}
              ...
              {!uploadedFile ? (
                <div>
                  <Button
                    ...
                    type="secondary" // or "default"
                  />
                  <div style={{ fontSize: textSizes.small, color: '#6b7280', marginTop: '4px' }}>(Max 2MB)</div>
                </div>
              ) : (
                <div>
                    ...
                    <div key={task.id} style={{ background: task.status === 'failed' ? '#fff1f2' : '#f8fafc', border: '1px solid #f1f5f9', borderRadius: '6px', padding: '8px', marginBottom: '6px', fontSize: textSizes.small }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px' }}>
                        ...
                        <span style={{ flex: 1, color: '#475569', fontWeight: '500', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                           {task.selectedFormatValue ? baseFormatOptions.find(f => f.value === task.selectedFormatValue)?.label.replace(/[📧•📊📄✅📋📅✏️{}]/g, '').trim() : task.outputFormat}
                          {task.hasFile && ' 📎'}
                        </span>
                        ...
                        <div style={{ position: 'relative' }}>
                          ...
                          {task.status === 'completed' && task.result && (
                            <Button 
                                ...
                                title="Insert result into draft"
                                size="small" // Assuming Button supports size
                                style={{ position: 'absolute', top: '4px', right: '4px' }} // Basic positioning
                            />
                          )}
                        </div>
                      )}
                  ...
                  {commentHistory.slice(0, 5).map((entry, index) => (
                    <div key={entry.timestamp + index} style={{ padding: '6px', background: '#f8fafc', border: '1px solid #f1f5f9', borderRadius: '4px', marginBottom: '4px', fontSize: textSizes.small }}>
                      ...
                      <div style={{ color: '#475569', lineHeight: 1.3, fontSize: textSizes.small, whiteSpace: 'pre-wrap', wordBreak: 'break-word', maxHeight: '4em', overflow: 'hidden', textOverflow: 'ellipsis' }} title={entry.text}>{entry.text}</div>
                       {entry.outputFormat && (<div style={{ fontSize: textSizes.tiny, color: '#6b7280', fontStyle: 'italic', marginTop: '2px' }}>Format: {entry.selectedFormatValue ? baseFormatOptions.find(f => f.value === entry.selectedFormatValue)?.label.replace(/[📧•📊📄✅📋📅✏️{}]/g, '').trim() : entry.outputFormat}</div>)}
                    </div>
                  ))}
                </div>
              </AccordionSection>
      ...
      <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '12px', flexShrink: 0 }}>
        <Button
          label={isSending ? 'Processing...' : (mode === 'email' ? 'Send Email Instructions' : 'Create Task')}
